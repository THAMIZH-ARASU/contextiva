...
**I want** the RAG query endpoint to optionally support Hybrid Search and Re-ranking,
**so that** I can improve the relevance of my search results (FR10).
...
5. E2E tests are updated to validate the behavior when these flags are enabled and disabled.
...
- [ ] Add Advanced RAG Configuration to Settings (AC: 4)
...
- [ ] Integration Tests for Keyword Search (AC: 5)
...
- [ ] Integration Tests for Updated Use Case (AC: 5)
  ...
  - [ ] Mock HybridSearchService and RerankingService [Source: architecture/test-strategy-and-standards.md]

- [ ] E2E Tests for Advanced RAG Features (AC: 5)
...
2. `QueryKnowledgeUseCase` - Add hybrid search and re-ranking logic
...
- `src/shared/config/settings.py` - Add advanced RAG settings
...
- `src/application/use_cases/knowledge/query_knowledge.py` - Add hybrid/reranking logic
